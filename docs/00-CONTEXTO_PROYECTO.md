...
El proyecto se encuentra en la **Fase 4 - API REST del Sistema** [95]. Se ha completado la implementación del backend con la API de negocio, la base de datos con todo el modelo relacional de la Fase 2 [102, 123], y la autenticación por sesión segura de la Fase 3 [112].
...
*   **Fase 5 (Frontend de Negocio & Pruebas E2E):** 🚧 **En curso — Sprint 1 y Sprint 2 cerrados** (Autenticación, Layout Shell, CRUDs Maestros, Importaciones, Documentos y Costeo); **Sprint 3 (Catálogo Mayorista y Pedidos) es la siguiente pendiente**. Consumo de la API REST desde la interfaz React por parte de los componentes de negocio (catálogo interactivo, creación de pedidos, gestor de importaciones) y automatización de pruebas de extremo a extremo (E2E) con Playwright [100, 101] — suite real y ejecutable desde `package.json` en la raíz (`npx playwright test`). Ver [`docs/04-SPRINTS.md`](04-SPRINTS.md) para la planificación detallada.
